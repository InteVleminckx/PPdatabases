...
class AB_Test:
    ...
    def to_dct(self):
        return {'test_id' : self.test_id, 'algorithm_id' : self.algorithm_id, 'start_point' : self.start_point, 'end_point' : self.end_point,
...
class Algorithm:
    ...
    def to_dct(self):
        return {'abtest_id' : self.abtest_id, 'algorithm_id' : self.algorithm_id, 'name' : self.name, 'parameters' : self.params}
...
def getClickTroughRate(abtest_id, algorithm_id, dataset_id, curDate):
    ...
    cursor = dbconnect.get_cursor()
    cursor.execute('SELECT count(DISTINCT r.item_number) FROM Recommendation r, Interaction i WHERE r.abtest_id = %s AND r.algorithm_id = %s AND %s BETWEEN r.start_point AND r.end_point'
                   ' AND i.dataset_id = %s AND i.t_dat = %s AND r.item_number = i.item_id;', (abtest_id, algorithm_id, curDate, dataset_id, curDate))
...
def addAB_Test(abtest_id, algorithm_id, start_point, end_point, stepsize, topk, creator, dataset_id):
    ...
    cursor = dbconnect.get_cursor()
    try:
        cursor.execute('INSERT INTO ABTest(abtest_id, algorithm_id, start_point, end_point, stepsize, topk, creator, dataset_id) VALUES(%s,%s,%s,%s,%s,%s,%s,%s)',
...
def getAB_Test(abtestId):
    global dbconnect
    cursor = dbconnect.get_cursor()
    cursor.execute('SELECT abtest_id, algorithm_id, start_point, end_point, stepsize, topk, creator, dataset_id FROM ABTest WHERE abtest_id = %s', (str(abtestId),))
...
def getItemRecommendations(retrainDay, item_id, abtest_id, dataset_id):
    ...
    for id in algorithmIDs:
        cursor.execute('SELECT count(*) FROM Recommendation WHERE abtest_id = %s AND algorithm_id = %s AND dataset_id = %s AND item_number = %s AND end_point = %s',
    ...
    for algorithm_id in algorithmIDs:
        ...
        if algorithm.name == 'popularity' or algorithm.name == 'recency':
            cursor.execute("SELECT COUNT(*) FROM Interaction i WHERE i.item_id = %s AND i.dataset_id = %s AND i.t_dat BETWEEN %s AND %s "
                           ...
                           (str(item_id), str(dataset_id), retrainDay, nextRetrainDay, str(abtest_id), str(algorithm_id), str(dataset_id), str(item_id), retrainDay))
