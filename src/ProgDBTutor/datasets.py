...
def getNumberOfUsers(cursor, dataset_id):
    ...
    query = 'CREATE OR REPLACE VIEW customers AS SELECT customer_number FROM Customer WHERE dataset_id = %s GROUP BY' \
            'customer_number; SELECT count(*) FROM customers; DROP VIEW customers;', (str(dataset_id),)
...
def getNumberOfArticles(cursor, dataset_id):
    #We doen dit op deze manier omdat dit sneller is als distinct, want distinct sorteerd eerst heel de tabel
    query = 'CREATE OR REPLACE VIEW articlesC AS SELECT item_number FROM Dataset WHERE dataset_id = %s GROUP BY' \
            'item_number; SELECT count(*) FROM articlesC; DROP VIEW articlesC;', (str(dataset_id),)
